...
uses
   SysUtils, Classes, Forms, StrUtils, DB, DBClient, SimpleDS,
...
begin
  FContigencia      := False;
  FAmbienteProducao := False; // Produção ou Homologação
  ...
  //----------------------------------------------------------------------
  FAmbienteProducao := FParametros.NFE.Ambiente = '1'; // 1=taProducao / 2=taHomologacao
...
procedure TBaseDados.QryVendaBeforePost(DataSet: TDataSet);
begin
   {***********************************************************************
    ** De acordo com o Ministério da Fazenda - 2012:                    ***
    **    1 – Simples Nacional                                          ***
   ...
   case db_Parametros.FieldByName('REGIME_TRIB').AsInteger of
    0, -1,1: begin
                ...
                begin
                   QryVenda.FieldByName('OBS1').AsString := UpperCase( fnCFOP_DETALHE(QryVenda.FieldByName('CFOP').AsInteger) );
