...
type
    TFrmClientesSifates = class(TForm)
...
var
   FrmClientesSifates: TFrmClientesSifates;
...
procedure TFrmClientesSifates.BtnComplementoClick(Sender: TObject);
begin
   //FrmClientesComplementos := TFrmClientesComplementos.Create(self);
   //FrmClientesComplementos.ShowModal;
end;

procedure TFrmClientesSifates.btnLiberarCreditoClick(Sender: TObject);
var
  mAcaousuario : string;
begin
...
procedure TFrmClientesSifates.btnRemoverClick(Sender: TObject);
begin
...
procedure TFrmClientesSifates.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmClientesSifates.FormCreate(Sender: TObject);
begin
  FrmFrameBotoes1.DataSource := DS_Clientes;
  FrmFrameBotoes1.DataSource.DataSet := DS_Clientes.DataSet;
end;

procedure TFrmClientesSifates.Iniciar;
begin
...
procedure TFrmClientesSifates.pMnuEnderecoCadastradoClick(Sender: TObject);
begin
  TelaEnderecos;
end;

procedure TFrmClientesSifates.GridComerciaisKeyPress(Sender: TObject; var Key: Char);
begin
   Key := AnsiUpperCase(Key)[1];
end;

procedure TFrmClientesSifates.GridFinanceiroDrawColumnCell(Sender: TObject;
...
procedure TFrmClientesSifates.GridBancosKeyPress(Sender: TObject; var Key: Char);
begin
   Key := AnsiUpperCase(Key)[1];

end;

procedure TFrmClientesSifates.db_ClientesAfterPost(DataSet: TDataSet);
begin
   if not (db_Clientes.State in [dsedit,dsinsert]  ) then
     exit;
...
procedure TFrmClientesSifates.db_ClientesBeforePost(DataSet: TDataSet);
begin
   if not (db_Clientes.State in [dsedit,dsinsert]  ) then
     exit;
   db_Clientes.ParamByname('CODIGO').AsInteger :=  db_Clientes.FieldByName('CODIGO').AsInteger;
...
procedure TFrmClientesSifates.db_ClientesNewRecord(DataSet: TDataSet);
begin
   if not (db_Clientes.State in [dsedit,dsinsert]  ) then
     exit;
...
procedure TFrmClientesSifates.db_ClientesUpdateRecord(DataSet: TDataSet;
  UpdateKind: TUpdateKind; var UpdateAction: TUpdateAction);
begin
  if (db_Clientes.FieldByName('ATIVO').NewValue<>db_Clientes.FieldByName('ATIVO').OldValue) then
   ...
   begin
      ...
      Begin
         ...
         FrmClientesSifates.EditCPF.SetFocus;
         Exit;

      End;

      strCNPJ := FrmClientesSifates.EditCPF.Text;
      If (db_Clientes.state in [dsEdit, dsInsert]) then
      Begin

         If fncCliente_cnpj(strCNPJ) then
         begin
            FrmClientesSifates.FrmFrameBotoes1.SpbCancelar.Click;
            BeepCritica;
            db_Clientes.Cancel;
   ...
   begin
      ...
      Begin
         ...
         FrmClientesSifates.EditCNPJ.SetFocus;
         Exit;
      End;

      strCNPJ := FrmClientesSifates.EditCNPJ.Text;
      If (db_Clientes.state in [dsEdit, dsInsert]) then
      Begin
...
procedure TFrmClientesSifates.db_ctareceberBeforeOpen(DataSet: TDataSet);
begin
   db_ctareceber.ParamByName('EMPRESA').AsInteger := FSistema.Empresa;

end;

procedure TFrmClientesSifates.db_ctareceberSumBeforeOpen(DataSet: TDataSet);
begin
   db_ctareceberSum.ParamByName('EMPRESA').AsInteger := FSistema.Empresa;
end;

procedure TFrmClientesSifates.db_PedidoAfterOpen(DataSet: TDataSet);
begin
  DimensionarGrid(GridPedidos,self);
end;

procedure TFrmClientesSifates.db_PedidoBeforeOpen(DataSet: TDataSet);
begin
  db_Pedido.ParamByName('EMPRESA').AsInteger := FSistema.Empresa;
end;

procedure TFrmClientesSifates.db_PedidoSumBeforeOpen(DataSet: TDataSet);
begin
  db_PedidoSum.ParamByName('EMPRESA').AsInteger := FSistema.Empresa;
end;

procedure TFrmClientesSifates.DadosEndereco;
Var
  mCEP   : TCEP;
begin
...
procedure TFrmClientesSifates.dbClienteAnexosBeforePost(DataSet: TDataSet);
begin
   QryAnexos.FieldByName('IDCLIENTE').AsInteger := db_Clientes.FieldByName('CODIGO').AsInteger;
end;

procedure TFrmClientesSifates.dbClienteCobrancasBeforePost(DataSet: TDataSet);
begin
  dbClienteCobrancas.FieldByName('IDCLIENTE').AsInteger := db_Clientes.FieldByName('CODIGO').AsInteger;
end;

procedure TFrmClientesSifates.dbPesquisarEnderecoChange(Sender: TObject);
begin
  DadosEndereco;
end;

procedure TFrmClientesSifates.dbPesquisarEnderecoClose(Sender: TObject);
begin
  DadosEndereco;
end;

procedure TFrmClientesSifates.DBEdit18KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmClientesSifates.DBEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
...
procedure TFrmClientesSifates.DBEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
...
procedure TFrmClientesSifates.DBEdit5KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
...
procedure TFrmClientesSifates.DBGrid12KeyPress(Sender: TObject; var Key: Char);
begin
  Key := AnsiUpperCase(Key)[1];
end;

procedure TFrmClientesSifates.Spb_email1Click(Sender: TObject);
var
  eMail: String;
begin
...
procedure TFrmClientesSifates.Spb_http1Click(Sender: TObject);
begin
...
{ pessoa fisica }
procedure TFrmClientesSifates.Tab_HistoricoExit(Sender: TObject);
begin
   db_Pedido.Close;
   db_ctareceber.Close;
   db_PedidoSum.Close;
...
procedure TFrmClientesSifates.RefreshTabelas;
begin
   db_Pedido.Close;
   db_ctareceber.Close;
   db_PedidoSum.Close;
...
procedure TFrmClientesSifates.rgBloqueioClick(Sender: TObject);
begin
...
procedure TFrmClientesSifates.rgTipoComercializarClick(Sender: TObject);
begin
  rgFormaComercializar.Enabled :=(rgTipoComercializar.ItemIndex=1);
end;

procedure TFrmClientesSifates.Tab_HistoricoShow(Sender: TObject);
begin
   RefreshTabelas;
end;

procedure TFrmClientesSifates.TelaEnderecos;
begin
  if not db_Clientes.Active then
     Exit;
...
procedure TFrmClientesSifates.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if Key = VK_F7 then
   begin
...
procedure TFrmClientesSifates.FormShow(Sender: TObject);
begin
  FrmFrameBotoes1.DataSource := DS_Clientes;
  FrmFrameBotoes1.DataSource.DataSet := DS_Clientes.DataSet;
...
procedure TFrmClientesSifates.FrmFrameBotoes1SpbAdicionarClick(Sender: TObject);
begin
   If (strCliente = 'J') then
   Begin
      PgClientes.ActivePage := Tab_PJuridica; { }
...
procedure TFrmClientesSifates.FrmFrameBotoes1SpbCancelarClick(Sender: TObject);
begin
   PgClientes.ActivePageIndex := 0;
...
procedure TFrmClientesSifates.FrmFrameBotoes1SpbEditarClick(Sender: TObject);
begin
   FrmFrameBotoes1.SpbEditarClick(Sender);
end;

procedure TFrmClientesSifates.FrmFrameBotoes1SpbExcluirClick(Sender: TObject);
begin
...
procedure TFrmClientesSifates.ImprimirPedidocomvalores1Click(Sender: TObject);
begin
   FrmFrameBotoes1SpbImprimirClick(Sender);
end;

procedure TFrmClientesSifates.ImprimirPedidosemvalores1Click(Sender: TObject);
begin
...
procedure TFrmClientesSifates.FrmFrameBotoes1SpbImprimirClick(Sender: TObject);
begin
...
procedure TFrmClientesSifates.FrmFrameBotoes1SpbProcurarClick(Sender: TObject);
var
  mID : Integer;
begin
   If (strCliente = 'J') then
...
procedure TFrmClientesSifates.HabilitarCampos;
Var
...
procedure TFrmClientesSifates.FrmFrameBotoes1SpbSairClick(Sender: TObject);
begin
   Close;
end;

procedure TFrmClientesSifates.FrmFrameBotoes1SpbSalvarClick(Sender: TObject);
begin
...
procedure TFrmClientesSifates.CodCidadeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFrmClientesSifates.cxGridAnexoCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TFrmClientesSifates.cxGridAnexoDblClick(Sender: TObject);
begin
  btnAbrirClick(Sender);
end;

Function TFrmClientesSifates.ChecarCliente(const aTipo, aDocumento : String) : Boolean;
Var
  mID : Integer;
begin
  result := false;
...
procedure TFrmClientesSifates.EdFantasiaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
...
procedure TFrmClientesSifates.EditAtividadeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
...
procedure TFrmClientesSifates.EditCNPJExit(Sender: TObject);
begin
  ChecarCliente('J', EditCNPJ.Text);
end;

procedure TFrmClientesSifates.EditCNPJKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
...
procedure TFrmClientesSifates.EditCodCidade1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmClientesSifates.EditCodCidade2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key=VK_HOME  then
   begin
...
procedure TFrmClientesSifates.EditCodevendedorChange(Sender: TObject);
begin
  if not (db_Clientes.State in [dsedit,dsinsert]  ) then
    exit;
...
procedure TFrmClientesSifates.EditCPFExit(Sender: TObject);
begin
  ChecarCliente('F', EditCPF.Text);
end;

procedure TFrmClientesSifates.EditNomeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
...
procedure TFrmClientesSifates.EdRGInscrEstKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
...
procedure TFrmClientesSifates.BitBtn1Click(Sender: TObject);
begin
...
procedure TFrmClientesSifates.BitBtn2Click(Sender: TObject);
begin
...
procedure TFrmClientesSifates.btnAbrirClick(Sender: TObject);
Var
...
procedure TFrmClientesSifates.btnAdicionarClick(Sender: TObject);
var
  fStream: TFileStream;
begin
...
procedure TFrmClientesSifates.EditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
  begin
