...
uses
  ...
  Classe.Global, dxDateRanges, Vcl.Mask, Vcl.DBCtrls;
...
type
   TFrmArquivoMorto = class(TForm)
    ...
    cxGridPedidosItensColumn6: TcxGridDBColumn;
    cxGridPedidosItensColumn7: TcxGridDBColumn;
    ed_pedido: TIDBEditDialog;
    ed_nomecliente: TDBEdit;
    Label1: TLabel;
...
procedure TFrmArquivoMorto.AprovarTudo1Click(Sender: TObject);
begin
...
procedure TFrmArquivoMorto.BtnaprovarClick(Sender: TObject);
begin
...
procedure TFrmArquivoMorto.BtndesaprovarClick(Sender: TObject);
begin
...
procedure TFrmArquivoMorto.BtnProcurarClick(Sender: TObject);
begin
   ...
   begin
      ...
      ed_pedido.Text := dbeditPedidoProcurar.ResultFieldAsString('CODIGO');
      ed_pedido.SetFocus;
   end;
...
procedure TFrmArquivoMorto.BtnsairClick(Sender: TObject);
begin
   close;
end;

procedure TFrmArquivoMorto.cxGridPedidosCustomDrawCell(
...
procedure TFrmArquivoMorto.cxGridPedidosItensCustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
begin
  if AViewInfo.GridRecord.Focused then
...
procedure TFrmArquivoMorto.dbGrid_ProdutosKeyDown(Sender: TObject;
...
procedure TFrmArquivoMorto.Desaprovartudo1Click(Sender: TObject);
begin
   ProgressBar1.Max :=db_Pedido.RecordCount;
   ProgressBar1.Position := 0;
   db_Pedido.first;
...
procedure TFrmArquivoMorto.EditPedidoChange(Sender: TObject);
begin
   db_Pedido.Locate('CODIGO', StrToIntDef(ed_pedido.Text,0), []);
end;

procedure TFrmArquivoMorto.EditPedidoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure TFrmArquivoMorto.FormActivate(Sender: TObject);
begin
   db_Pedido.Refresh;
...
procedure TFrmArquivoMorto.rdgFiltroPedidosClick(Sender: TObject);
begin
   prcFiltrarPedidos;
   ed_pedido.SetFocus;
end;

procedure TFrmArquivoMorto.db_ChecarAtrasosBeforeOpen(DataSet: TDataSet);
begin
...
procedure TFrmArquivoMorto.prcFiltrarPedidos;
begin
  ...
  db_Pedido.SQL.Clear;
  db_Pedido.SQL.Add('SELECT * FROM VIEW_SIFATES_PEDIDOS  ');
