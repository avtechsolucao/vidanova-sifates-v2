...
uses
   ...
   Dialogs, DB, StdCtrls, Buttons, ExtCtrls, ComCtrls, Grids, DBGrids,
   ...
   frxClass, frxDBSet,  DBCGrids, Menus, FFrameBarra, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf,
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.Client,
  FireDAC.Comp.DataSet, GradeProduto;
...
type
   ...
   TFrmFichaTecnica = class(TForm)
    ...
    procedure EditGradeTamanhosExit(Sender: TObject);
   private
...
uses FPrincipal, SQLServer, Biblioteca, Global, FBaseDados_PCP,
  FFichaTecnicaGradeFoto, FFichaTecnicaImprimir, Classe.Global;
...
begin
   ...
   FrmPrincipal.actPCPFichaTecnica.Enabled := false;
...
begin
   FrmPrincipal.actPCPFichaTecnica.Enabled := True;
...
begin
 try
   ...
   if fileexists(BaseDados_PCP.db_referencia_Fotos.FieldByname('FOTO_PATH')
     .AsString) then
   Begin
      FotoReferencia.Picture.LoadFromFile(BaseDados_PCP.db_referencia_Fotos.FieldByname('FOTO_PATH').AsString);
   End
   Else
   Begin
...
begin
   ...
   begin
     ...
     dbEditPesquisarCoresTecidos.SearchQuery.Add('CODIGO='+GetProdutoCampo(BaseDados_PCP.db_MateriaPrimaTecidos.FieldByName('REFERENCIA').AsString,'CODIGO') );
...
begin
   ...
   begin
     ...
     begin
        ...
        editPesquisaMateriaCor.SearchQuery.Add('CODIGO='+GetProdutoCampo(BaseDados_PCP.db_MateriaPrimaGrade.FieldByName('REFERENCIA').AsString,'CODIGO') );
     ...
     begin
        ...
        editPesquisaMateriaTamanho.SearchQuery.Add('CODIGO='+GetProdutoCampo(BaseDados_PCP.db_MateriaPrimaGrade.FieldByName('REFERENCIA').AsString,'CODIGO') );
...
procedure TFrmFichaTecnica.EditGradeTamanhosExit(Sender: TObject);
begin
   FIDGrade:=BaseDados_PCP.db_referencias.FieldByName('GRADE').AsInteger;

   db_GradeLista.Close;
   db_GradeLista.ParamByName('GRADE').Clear;;
   db_GradeLista.ParamByName('GRADE').AsInteger := FIDGrade;
   db_GradeLista.open;
...
begin
   ...
   if empty(BaseDados_PCP.db_referencias.FieldByname('REFERENCIA').AsString) Then
   begin
      TratarErros('Informe o codigo da Referencia.');
      EditReferencia.SetFocus;
      Abort;
   end;
   ...
   FID := BaseDados_PCP.db_referencias.FieldByname('CODIGO').AsInteger;
   ...
   if (BaseDados_PCP.db_referencias.FieldByname('MARKUPID').AsInteger>0) then
   begin
      Try
         SP_MarkupAplicar.ParamByName('CODIGO').AsInteger  := BaseDados_PCP.db_referencias.FieldByname('CODIGO').AsInteger;
         SP_MarkupAplicar.ParamByName('NMARKUP').AsInteger := BaseDados_PCP.db_referencias.FieldByname('MARKUPID').AsInteger;
   ...
   Reabrirtabela;
   BaseDados_PCP.db_referencias.Close;
   BaseDados_PCP.db_referencias.ParamByName('CODIGO').AsInteger := FID;
   BaseDados_PCP.db_referencias.open;
   ...
   //----------------------------------------------------------------------
   if (Trim(BaseDados_PCP.db_referencias.FieldByname('REFERENCIA').AsString)>'') and (BaseDados_PCP.db_referencias.FieldByName('TIPO_PRODUTO').AsString='ACA') then
   begin
     try
       GradeProduto1.Visible := true;
       GradeProduto1.IDEmpresa          := FSistema.Empresa;
       GradeProduto1.Produto.IDProduto  := BaseDados_PCP.db_referencias.FieldByName('CODIGO').AsInteger;
       GradeProduto1.Produto.Referencia := BaseDados_PCP.db_referencias.FieldByname('REFERENCIA').AsString;
...
begin
   ...
   db_ReferenciaGrade.open;
    ...
    frxFichaCusto.Variables['rptPrint']    := QuotedStr ('');
    frxReportImagem.Variables['rptPrint']   := QuotedStr ('');
    if FileExists(BaseDados_PCP.db_referencia_Fotos.FieldByname('FOTO_PATH').AsString) then
    begin
      frxFichaCusto.Variables['rptPrint']     := QuotedStr ('SIM');
      frxFichaCusto.Variables['rptCaminho']   := QuotedStr (BaseDados_PCP.db_referencia_Fotos.FieldByname('FOTO_PATH').AsString);

      frxReportImagem.Variables['rptPrint']   := QuotedStr ('SIM');
      frxReportImagem.Variables['rptCaminho'] := QuotedStr (BaseDados_PCP.db_referencia_Fotos.FieldByname('FOTO_PATH').AsString);
    end;
